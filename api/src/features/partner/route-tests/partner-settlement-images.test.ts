...
describe("GET /api/partner/settlements/:settlementId/images/:imageId", () => {
	beforeEach(async () => {
	...
	it("パートナーの精算画像をダウンロードできる", async () => {
		await insertPartnership(TEST_USER.id, OTHER_USER.id);
		const settlement = await insertSettlement(OTHER_USER.id);
		const { imageId } = await uploadImageAsPartner(settlement.id);

		const res = await app.request(
			`/api/partner/settlements/${settlement.id}/images/${imageId}`,
	...
	it("自分の精算の画像はパートナーエンドポイントでは取得できない", async () => {
		await insertPartnership(TEST_USER.id, OTHER_USER.id);
		...
		const imgRes = await app.request(
			`/api/settlements/${settlement.id}/images`,
		...
		const res = await app.request(
			`/api/partner/settlements/${settlement.id}/images/${imgBody.id}`,
			{ headers: { Cookie: authCookie } },
			env,
		);
	...
	it("存在しない画像IDは 404 を返す", async () => {
		...
		const res = await app.request(
			`/api/partner/settlements/${settlement.id}/images/nonexistent`,
			{ headers: { Cookie: authCookie } },
			env,
		);
	...
	it("認証なしでリクエストすると 401 を返す", async () => {
		const res = await app.request(
			"/api/partner/settlements/any/images/any",
			{},
			env,
		);
