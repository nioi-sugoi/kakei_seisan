...
- **アップロード**: クライアントが API に `multipart/form-data` で画像を送信 → API が R2 に保存（Workers 経由の直接アップロード）
- **ダウンロード**: クライアントが API 経由で画像を取得（API が R2 から読み出してプロキシ）
- **削除**: API 経由で画像を削除（DB レコード削除 → R2 オブジェクト削除の順）
- **アクセス制御**: 全操作で API 側の認証ミドルウェアにより認証・認可チェック
- **パス規約**: `receipts/{user_id}/{entry_id}/{uuid}.{ext}`
- **制約**: 1 レコードあたり最大 2 枚、1 ファイル 10MB 以下、対応形式は JPEG / PNG / WebP / HEIC
